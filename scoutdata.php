...
if ($action == "getmatches" || $action == "getrawmatches") {
	...
	$statement = $db->prepare("SELECT `stand_scouting`.*, `total_points`.`total_points` FROM `stand_scouting` INNER JOIN `total_points` ON `total_points`.`match_number` = `stand_scouting`.`match_number` WHERE `stand_scouting`.`team_number`=:team_number ORDER BY `stand_scouting`.`match_number` ASC");
	$statement->bindParam(":team_number", $team, PDO::PARAM_INT);
	$success = $statement->execute();
	http_response_code(200);
